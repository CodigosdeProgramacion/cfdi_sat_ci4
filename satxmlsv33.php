<?php
	...
	// |               de 2016.                                                    |
	// |                                                                           |
	// |  Nota: El 1 de julio de 2017 entró en vigor la versión 3.3 de la factura; |
	// |        siendo obligatoria a partir del 1 de enero del 2018.               |
	// |        No incluye addenda                                                 |
	// +---------------------------------------------------------------------------+
	...
	// +---------------------------------------------------------------------------+
	// |15/nov/2020 Se toma como base el programa de la version 2.2               |
	// +---------------------------------------------------------------------------+
	//
	...
	function satxmlsv33($arr, $edidata=false, $dir="./tmp/",$nodo="") {
		...
		satxmlsv33_genera_xml($arr,$edidata,$dir,$nodo);
	...
	function satxmlsv33_genera_xml($arr, $edidata, $dir,$nodo) {
		...
		satxmlsv33_generales($arr, $edidata, $dir,$nodo);
		satxmlsv33_emisor($arr, $edidata, $dir,$nodo);
		satxmlsv33_receptor($arr, $edidata, $dir,$nodo);
		satxmlsv33_conceptos($arr, $edidata, $dir,$nodo);
	...
	function satxmlsv33_generales($arr, $edidata, $dir,$nodo) {
		...
		satxmlsv33_cargaAtt($root, array("Version"=>$arr['version'],
		"Serie"=>$arr['serie'],
		"Folio"=>$arr['folio'],
		...
		"FechaFolioFiscalOrig"=>satxmlsv33_xml_fech($arr['fechaFolioFiscalOrig']),
	...
	function satxmlsv33_conceptos($arr, $edidata, $dir, $nodo) {
		...
		for ($i=0; $i<sizeof($arr['conceptos']); $i++) {
			...
			satxmlsv33_cargaAtt($concepto, array("ClaveProdServ"=>$arr['conceptos'][$i]['clave'],
			...
			if (isset($arr['conceptos'][$i]['iBase'])) {
				...
				satxmlsv33_cargaAtt($traslado, array("Base"=>$arr['conceptos'][$i]['iBase'],
	...
	function satxmlsv33_impuestos($arr, $edidata, $dir,$nodo) {
		...
		if (isset($arr['traslados']['importe'])) {
			...
			$traslado = $xml->createElement("cfdi:Traslado");
			$traslado = $traslados->appendChild($traslado);
			satxmlsv33_cargaAtt($traslado, array("Impuesto"=>$arr['traslados']['impuesto'],
			"TipoFactor"=>'Tasa',
